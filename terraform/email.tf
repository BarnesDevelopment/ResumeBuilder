...
resource "fusionauth_email" "forgot-password" {
  name                  = "${local.name} - Forgot Password"
  default_from_name     = local.default_from_name
  default_html_template = file("${local.email_templates_path}/Forgot_Password.html.ftl")
...
resource "fusionauth_email" "setup-password" {
  name                  = "${local.name} - Setup Password"
  default_from_name     = local.default_from_name
  default_html_template = file("${local.email_templates_path}/Setup_Password.html.ftl")
  default_subject       = "Setup Password"
  default_text_template = file("${local.email_templates_path}/Setup_Password.txt.ftl")
  from_email            = local.noreply_address
}
