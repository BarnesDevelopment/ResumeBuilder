...
public class ResumeOrchestrator : IResumeOrchestrator
{
  ...
  public ResumeOrchestrator(IResumeBuilderService builderService, IResumeTree tree, IResumeService resumeService)
  {
  ...
  public async Task<ResumeTreeNode?> GetResumeTree(Guid id)
  {
    ...
    return await _resumeService.GetFullResumeTree(root);
  }
  ...
  public async Task<ResumeTreeNode> UpsertNode(ResumeTreeNode resume, Guid userId)
  {
    ...
    if (resume.Children != null && resume.Children.Any()) resume = await CreateResumeRecurseChildren(resume, userId);
  ...
  private List<TagBuilder> BuildExperience(List<ResumeExperience> experience)
  {
    ...
    {
      _builderService.AddSeparator("Experience")
    };
    ...
    foreach (var job in experience)
    {
      ...
      jobHeader.InnerHtml.AppendHtml(TagHelper.CreatTag("span", "spacer", "-"));
      jobHeader.InnerHtml.AppendHtml(TagHelper.CreatTag("span", "end",
  ...
  private TagBuilder BuildEducation(List<ResumeEducation> education)
  {
    ...
    foreach (var school in education) BuildSchool(school, educationTag);
